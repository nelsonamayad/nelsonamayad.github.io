...
subtitle: "Plato logístico para la 2da vuelta"
author: "Nelson Amaya"
output: 
  html_document:
      theme: readable
...
Este último plato busca lograr lo que los anteriores no pudieron: estimar qué tan probable es que gane uno u otro candidato. Para hacer eso este plato le aplica una transformación sencillita al plato [Simple](https://nelsonamayad.github.io/mixto/mixto): en vez de servir dos recetas por candidato, se sirve una sola [logística](https://en.wikipedia.org/wiki/Logistic_regression) para ambos. Esto permite generar un pronóstico probabilístico para la segunda vuelta.
...
## Plato Logis
<br>
Este plato recoge la probabilidad de que [`@IvanDuque`](https://twitter.com/IvanDuque) sea presidente, estimada por el modelo que se describe abajo. 
...
> Según este plato logístico [`@IvanDuque`](https://twitter.com/IvanDuque) tiene más o menos 65% de probabilidad de obtener más del 50% de la votación y ganar las elecciones.
...
### Receta
<br>
Este plato modela directamente las encuestas como una distribución binomial donde cada encuesta es un ensayo:
...
## Preparación
<br>
Este es el plato completo, con los *priors* del caso:
<br>
$$ N^{Duque}_i \sim \textrm{Binomial}(N_i, \pi^{Duque}_i) $$
$$\textrm{logit} (\pi^{Duque}_i) = \alpha_{encuestadora}$$
...
```
<br>
Ahora meter el plato al horno de RStan:
<br>
...
```
<br>
Con algo de trabajo, y a pesar de muchas divergencias iniciales en el muestreo, el plato no se quema y sale del horno. 
...
<br>
Además de la siempre abstrusa representación del código en RStan, en este último plato incluyo una alternativa que de paso le hace bombo al paquete [rethinking](http://xcelab.net/R/rethinking_package.pdf) que preparó uno de los mejores cocineros: [Richard McElreath](http://xcelab.net/rm/).
