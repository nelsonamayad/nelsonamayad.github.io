...
## Plato
<br>
Primero veamos cómo se comparan los resultados del segundo plato *mixto* con los del primer plato [*simple* ](https://nelsonamayad.github.io/receta). Solo se comparan los promedios para facilitar la visualización; eso no significa que cada punto del plato mixto no tenga su correspondiente intervalo de credibilidad, solo que ese intervalo no se muestra. 

El plato mixto, con las mismas encuestas, cambia considerablemente las estimaciones para [Gustavo Petro](https://petro.com.co/) (+5 pp), y en menor medida para [Sergio Fajardo](http://sergiofajardo.co/) (+2.3 pp) y para [Germán Vargas Lleras](https://www.mejorvargaslleras.com/) (-2.7 pp). Para los otros candidatos la diferencia es mínima. Eso resulta de cómo el modelo procesa la información de las relaciones que hay entre las caracterísitcas de las encuestas, como su tamaño o márgen de error, algo que no se puede resumir en una frase.

<br>
```{r, echo=F, message=F}
library(tidyverse)
#Platos 1 y 2:
platos <- tribble(~candidato,~int_voto,~m_2018,~m_03.2018,~plato,
  "Sergio Fajardo",10.6,15.3,12.4,"Plato Simple",
  "Gustavo Petro",20.9,19.6,24.7,"Plato Simple",
  "Ivan Duque",36.2,16.5,36.9,"Plato Simple",
  "Humberto de la Calle",3.4,5.4,3.9,"Plato Simple",
  "German Vargas Lleras",10.4,8.9,7.4,"Plato Simple",
  "Sergio Fajardo",13.0,15.3,12.4,"Plato Mixto",
  "Gustavo Petro",26.0,19.6,24.7,"Plato Mixto",
  "Ivan Duque",35.9,16.5,36.9,"Plato Mixto",
  "Humberto de la Calle",3.7,5.4,3.9,"Plato Mixto",
  "German Vargas Lleras",7.7,8.9,7.4,"Plato Mixto")
...
## Ingredientes
<br>
Esta receta utiliza la misma información que la anterior: las **9 encuestas** que han salido después de las elecciones legislativas del 11 de marzo.
...
El plato mixto utiliza un prior específico con la distribución [Lewandowski-Kurowicka-Joe, o LKJ para sus amigos)](https://www.sciencedirect.com/science/article/pii/S0047259X09000876), que permite definir con un sólo parámetro $\eta$ la diagnonal de la matriz de correlación. 
...
duque_fit <- stan(file='duque.stan',data=list(N=9,N_encuestadora=6,int_voto=id$int_voto,encuestadora=id$encuestadora, muestra_int_voto=id$muestra_int_voto,m_error=id$m_error,dd=id$dd,tipo=id$tipo),control=list(adapt_delta=0.9),iter=4000,chains=4)
```
<br>
...
mcmc_trace(posterior,pars=c("m[1]","m[2]","m[3]","m[4]","m[5]","m[6]","m[7]","m[8]","m[9]"))
```
<br>
...
par <- posterior %>% as.tibble() %>% select(127:135) %>% summarize_all(funs(mean)) %>% t()
obs <- tibble(int_voto = id$int_voto,n = seq(from=1,to=9,by=1))
...
ggplot(par_obs, aes(x=n))+
    ...
    theme_classic()+labs(y="intencion de voto",x="")+scale_x_continuous(breaks=c(1,2,3,4,5,6,7,8,9))+
    ylim(values=c(30,50))
```
<br>

Este plato tiene muchísimos parámetros (137 en total, por los efectos mixtos), así que abajo sólo presento los 9 más relevantes: 
<br>
...
posterior <- as.matrix(duque_fit)
color_scheme_set("orange")
mcmc_areas(posterior,prob=0.9,prob_outer = 0.99,point_est="mean",pars=c("m[1]","m[2]","m[3]","m[4]","m[5]","m[6]","m[7]","m[8]","m[9]"))
```
<br>
<br>
