...
platos <- tribble(~candidato,~int_voto,~m_2018,~m_03.2018,~plato,
  "Sergio Fajardo",11.8,13.7,11.6,"Plato Simple",
  "Gustavo Petro",19.8,22.7,26.2,"Plato Simple",
  "Ivan Duque",37.3,25.7,38.8,"Plato Simple",
  "Humberto de la Calle",2.5,4.5,3.3,"Plato Simple",
  "German Vargas Lleras",7.3,8.3,7.1,"Plato Simple",
  "Sergio Fajardo",11.1,13.7,11.6,"Plato Mixto",
  "Gustavo Petro",28.5,22.7,26.2,"Plato Mixto",
  "Ivan Duque",37.9,25.7,38.8,"Plato Mixto",
  "Humberto de la Calle",2.8,4.5,3.3,"Plato Mixto",
  "German Vargas Lleras",7.4,8.3,7.1,"Plato Mixto")
...
duque_fit <- stan(file='duque.stan',data=list(N=10,N_encuestadora=6,int_voto=id$int_voto,encuestadora=id$encuestadora, muestra_int_voto=id$muestra_int_voto,m_error=id$m_error,dd=id$dd,tipo=id$tipo),control=list(adapt_delta=0.9),iter=4000,chains=4)
```
<br>
...
par <- posterior %>% as.tibble() %>% select(127:136) %>% summarize_all(funs(mean)) %>% t()
obs <- tibble(int_voto = id$int_voto,n = seq(from=1,to=10,by=1))
...
posterior <- as.matrix(duque_fit)
color_scheme_set("orange")
mcmc_areas(posterior,prob=0.9,prob_outer = 0.99,point_est="mean",pars=c("m[1]","m[2]","m[3]","m[4]","m[5]","m[6]","m[7]","m[8]","m[9]","m[10]"))
```
<br>
<br>
