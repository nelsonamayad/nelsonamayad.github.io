...
<br>
<br>
*Última actualizacion:* 2018-04-27
<br>
- Encuestas totales: *37*
<br>
- Encuestas durante 2018: *22*
<br>
- Encuestas después de las elecciones legislativas: *10*
<br>
...
Una encuesta con bajo poder no es capaz de identificar diferencias estadísticamente significativas entre la intención de voto de los candidatos que busca medir.
...
El poder representa la probabilidad de que un test estadístico detecte un efecto cuando ese efecto realmente existe. Así, cuando un test tiene bajo poder, no es capaz de identificar o detectar lo que pretende. 
...
- La correlación intra-cluster ($\rho$) y número de clusters. No se incluye en este análisis. (Pero se debería incluir.) 

## ¿Cómo se calcula el poder de una encuesta?

Ya que algunas fichas técnicas de las encuestas reportan márgenes de error para cierto tipo de pruebas (e.g. *"el margen de error para una confianza del Y% es X% para proporciones mayores a Z%"*), se interpretan esos márgenes para calcular el poder *post hoc* de cada encuesta utilizando la muestra efectiva, el tipo de prueba (i.e. diferencias entre proporciones), el nivel de significancia (casi siempre 95%) y se deja como variable el tamaño del efecto (de 1%, 5% y 10% de diferencia).
...
En un paso se sube la información de las encuestas. Luego,
...
#1.1 Diferencias de proporciones 0.5 para encuestas #: 3-4, 6-9, 16, 29, 33 y Default: 1,10,12,14,18,21,22,24,25,28,31,32,35
cons.1 <- 0.5       
gr1 <- c(1,3,4,6,7,8,9,10,12,14,16,18,21,22,24,25,26,28,29,31,32,33,35)
...
#1.7 Diferencias de proporciones 0.28 para encuestas #37
cons.7 <- 0.28     #Encuestas: 37
gr7 <- 37
  
#2. Tamaños de los efectos
eff.1 <- 0.01   #Efecto a detectar: Diferencia de 1%
eff.2 <- 0.05   #Efecto a detectar: Diferencia de 5%
eff.3 <- 0.1    #Efecto a detectar: Diferencia de 10%
...
Utilizando el paquete [**pwr**](https://cran.r-project.org/web/packages/pwr/pwr.pdf) de R y, el más ineficiente código imaginable, el poder de cada encuesta para detectar una diferencia de 1% entre la votación de los candidatos se calcula así:
...
#7. Grupo 7, Efecto 1 ####
pow.37 <- pwr.p.test(h = ES.h(eff.1+cons.7, cons.7), sig.level = a, alternative = "greater", n = encuestas$muestra_int_voto[37])
...
#7. Grupo 7, Efecto 3 ####
pow.37 <- pwr.p.test(h = ES.h(eff.2+cons.7, cons.7), sig.level = a, alternative = "greater", n = encuestas$muestra_int_voto[37])
...
#### Poder 10%: Efecto grande de diferencia

¿Es la muestra de las encuestas suficiente para detectar una diferencia de 10% entre candidatos?
...
#7. Grupo 7, Efecto 3 ####
pow.37 <- pwr.p.test(h = ES.h(eff.3+cons.7, cons.7), sig.level = a, alternative = "greater", n = encuestas$muestra_int_voto[37])
...
El estándar de poder (arbitrario, como todo lo frecuentista) para un test es **80%**. Eso quiere decir que para detectar un efecto que existe, se detectará el 80% de las veces que se busque con una muestra de ese tamaño, para determinado tamaño del efecto y con 95% de nivel de significancia. 
...
#3. Grafica:
ggplot(df, aes(x=power,y=muestra_int_voto,group=encuestadora))+
  ...
  labs(y="Muestra de la encuesta", 
       ...
       subtitle="Poder post hoc de las encuestas (N = 37) para detectar diferencias de 1%, 5% y 10%",
       caption="Fuente: Calculos en R usando test pwr.p.test"
       ) +
...
> Las encuestas son muy pequeñas para detectar efectos pequeños en una carrera pareja, pero son lo suficientemente grandes para detectar diferencias de 10% entre los candidatos. Eso es normal.
...
ggplot(pleb,aes(x=fecha,y=si))+
  ...
  labs(x="",y="% Votos 'Si'",
       title="El descache de las encuestas en el plebistico",        subtitle="12 encuestas antes del plebiscito 2016 entre 18/08/2016 y 27/09/2016 con sus márgenes de error", 
       caption="https://es.wikipedia.org/wiki/Plebiscito_sobre_los_acuerdos_de_paz_de_Colombia_de_2016")
```

Utilizando el mismo método anterior, es posible responder qué tan grande hubieran tenido que ser las encuestas en la previa del plebiscito para detectar una diferencia de 0.43% en el resultado, con un poder de detección de 80% y bajo la hipótesis de que el Si iba a ganar (51%):

```{r, echoe=FALSE,message=FALSE}
library(pwr)
pwr.p.test(h=ES.h(0.5+0.0043, 0.51), sig.level=0.05, power=0.8)
```

Para detectar el resultado del plebiscito en 4 de cada 5 intentos, las encuestas debieron encuestar a cerca de **60 mil personas**.
