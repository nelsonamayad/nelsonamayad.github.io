...
eff.1 <- 0.02   #Efecto a detectar: Diferencia de 2%
eff.2 <- 0.05   #Efecto a detectar: Diferencia de 5%
eff.3 <- 0.08    #Efecto a detectar: Diferencia de 8%
...
#### Poder 2%: Efecto diminuto de diferencia

¿Es la muestra de las encuestas suficiente para detectar una diferencia de 2% entre candidatos?
...
#### Poder 8%: Efecto grande de diferencia

¿Es la muestra de las encuestas suficiente para detectar una diferencia de 8% entre candidatos?
...
El estándar de poder (arbitrario, como todo lo frecuentista) para un test es **80%**. Eso quiere decir que para detectar un efecto que existe, para determinado tamaño del dicho efecto y con 95% de nivel de significancia, será posible identificar ese efecto el 80% de las veces con un tamaño de muestra específica. 

Este es el poder estadístico estimado para cada encuesta en cada uno de los 3 diferentes tamaños de efectos:
...
#3. Grafica:
ggplot(df %>% filter(n>16), aes(x=power,y=muestra_int_voto,group=encuestadora))+
  ...
  labs(y="Muestra de la encuesta*", 
       ...
       subtitle="Poder post hoc de las encuestas (N = 22) para detectar diferencias de 2%, 5% y 8%",
       caption="*Muestra que reporta la intención de voto, no necesariamente la muestra total \nFuente: Calculos en R usando test pwr.p.test"
       ) +
...
> Las encuestas son muy pequeñas para detectar efectos pequeños en una carrera pareja, pero son lo suficientemente grandes para detectar diferencias de más de 8% entre los candidatos. Eso es normal.
...
Para detectar el resultado del plebiscito en 4 de cada 5 intentos, las encuestas debieron encuestar a cerca de **47 mil personas**.
