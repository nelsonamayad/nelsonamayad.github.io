...
<br>
Última actualización: **19-04-2018**: Estimaciones con nueva encuesta.
<br>
<br>
...
## Plato
<br>
Estos son los resultados del modelo. Son promedios de la distribución posterior estimada para cada candidato, así como los intervalos HPD (higher posterior density) de 90% sobre esos parámetros. Para tener una referencia, el resultado se compara con dos promedios simples: uno sobre las 18 encuestas disponibles a la fecha que se han hecho durante 2018, y otro sobre las **8 encuestas** que se han hecho después de las elecciones legislativas del 11 de marzo.
...
plato <- tribble(~candidato,~int_voto,~int_voto_max,~int_voto_min,~m_2018,~m_03.2018,
            "Sergio Fajardo",10.7,15.1,5.5,15.3,12.4,
            "Gustavo Petro",20.8,23.1,18.4,19.6,24.7,
            "Ivan Duque",36.1,43.4,30.5,16.5,36.9,
            "Humberto de la Calle",3.3,5.6,1.2,5.4,3.9,
            "German Vargas Lleras",10.3,12.6,8.1,8.9,7.4)
...
fajardo_fit <- stan(file='fajardo.stan',data=list(N=8,N_encuestadora=6,int_voto=sf$int_voto,encuestadora=sf$encuestadora, muestra_int_voto=sf$muestra_int_voto,m_error=sf$m_error,dd=sf$dd,tipo=sf$tipo), control=list(adapt_delta=0.9),
iter=4000,chains=4)
```
<br>
...
mcmc_trace(posterior,pars=c("m[1]","m[2]","m[3]","m[4]","m[5]","m[6]","m[7]","m[8]"))

# Parámetros vs Observados
par <- posterior %>% as.tibble() %>% select(15:22) %>% summarize_all(funs(mean)) %>% t()
...
ggplot(par_obs, aes(x=n))+
  ...
  theme_classic()+labs(y="intencion de voto",x="")+scale_x_continuous(breaks=c(1,2,3,4,5,6,7,8))
```
<br>
