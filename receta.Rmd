...
<br>
<br>
Este es un juguete simple para las pocas encuestas de intención de voto que se hacen sobre las elecciones presidenciales de Colombia en 2018. Combina tres cosas chéveres: estadística bayesiana, reproducibilidad y computación open-source. Las tendencias en las encuestas se consultan mejor [acá](https://nelsonamayad.shinyapps.io/col2018_tend/).
...
## Plato
<br>
Estos son los resultados del modelo. Son promedios de la distribución posterior estimada para cada candidato, así como los intervalos HPD (higher posterior density) de 90% sobre esos parámetros. Para tener una referencia, el resultado se compara con dos promedios simples: uno sobre las 18 encuestas disponibles a la fecha que se han hecho durante 2018, y otro sobre las 7 encuestas que se han hecho después de las elecciones legislativas del 11 de marzo.
...
mcmc_trace(posterior,pars=c("m[1]","m[2]","m[3]","m[4]","m[5]","m[6]","m[7]"))

# Parámetros vs Observados
par <- posterior %>% as.tibble() %>% select(15:21) %>% summarize_all(funs(mean)) %>% t()
obs <- tibble(int_voto = sf$int_voto,
              n = seq(from=1,to=7,by=1))
...
ggplot(par_obs, aes(x=n))+
  ...
  theme_classic()+labs(y="intencion de voto",x="")+scale_x_continuous(breaks=c(1,2,3,4,5,6,7))
```
<br>
