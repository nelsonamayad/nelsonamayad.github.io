...
#2. Alistamiento #####
df <- encuestas %>% select(n,fecha, encuestadora,ivan_duque,gustavo_petro,sergio_fajardo,german_vargas_lleras, humberto_delacalle, margen_error, muestra_int_voto)
...
# Comunes denominadores graficas ####
shape_enc <- c(4,16,17,3,15,25,8)
subtitle <- c("23 encuestas durante 2018 \n11 desde las elecciones legislativas del 11 de marzo de 2018")
fuente <- c("Fuente: Calculos propios con base en encuestas publicamente disponibles")
legis <-  geom_vline(xintercept=as.Date("2018-03-11"), size=0.1,linetype="dashed")

# 3 La grafica ####
ggplot(df,aes(x=fecha, y=int_voto)) +
  #Loess
  geom_smooth(aes(fill=candidato, color=candidato, weight=muestra_int_voto), method="loess", show.legend = F) +
  #Encuestas
...
Las encuestas son la única alternativa a la intuición y el chisme. Tienen errores, algunos explícitos y otros implícitos. Usan diferentes metodologías y estrategias muestrales, unas mejores que otras. Algunas revelan la tasa de respuesta; otras se la guardan. [No suelen tener mucho poder estadístico](https://nelsonamayad.github.io/poder/poder). Hay muchas razones para cuestionar, incluso desconfiar, de las encuestas. Pero hay más razones para desconfiar de analistas y columnistas: a la carreta casi nunca se le lleva contabilidad.
