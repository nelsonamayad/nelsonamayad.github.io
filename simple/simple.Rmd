...
## Plato
<br>
Estos son los resultados del modelo. Son promedios de la distribución posterior estimada para cada candidato, así como los intervalos HPD (higher posterior density) de 90% sobre esos parámetros. Para tener una referencia, el resultado se compara con dos promedios simples: uno sobre las encuestas disponibles a la fecha que se han hecho durante 2018, y otro sobre las **14 encuestas** que se han hecho después de las elecciones legislativas del 11 de marzo.
...
plato <- tribble(~candidato,~int_voto,~int_voto_max,~int_voto_min,~m_2018,~m_03.2018,
            "Sergio Fajardo",13.2,15,11.1,13.8,12.4,
            "Gustavo Petro",22.9,25.5,20.5,23.5,26.5,
            "Ivan Duque",34,37.7,30.6,27.5,38.3,
            "Humberto de la Calle",3.2,4.6,1.7,4.2,3.2,
            "German Vargas Lleras",12.4,13.4,11.1,8.4,7.5)
...
fajardo_fit <- stan(file='fajardo.stan',data=list(N=13,N_encuestadora=6,int_voto=sf$int_voto,encuestadora=sf$encuestadora, muestra_int_voto=sf$muestra_int_voto,m_error=sf$m_error,dd=sf$dd,tipo=sf$tipo),control=list(adapt_delta=0.9),iter=4000,chains=4)
```
<br>
...
par <- posterior %>% as.tibble() %>% select(15:27) %>% summarize_all(funs(mean)) %>% t()
obs <- tibble(int_voto = sf$int_voto,
                n = seq(from=1,to=13,by=1))
