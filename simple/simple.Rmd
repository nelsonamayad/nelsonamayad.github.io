...
<br>
Última actualización: **23-04-2018**. *Encuesta Guarumo*.
<br>
<br>
...
## Plato
<br>
Estos son los resultados del modelo. Son promedios de la distribución posterior estimada para cada candidato, así como los intervalos HPD (higher posterior density) de 90% sobre esos parámetros. Para tener una referencia, el resultado se compara con dos promedios simples: uno sobre las 18 encuestas disponibles a la fecha que se han hecho durante 2018, y otro sobre las **9 encuestas** que se han hecho después de las elecciones legislativas del 11 de marzo.
...
plato <- tribble(~candidato,~int_voto,~int_voto_max,~int_voto_min,~m_2018,~m_03.2018,
            "Sergio Fajardo",10.6,14.6,6.7,15.3,12.4,
            "Gustavo Petro",20.9,23.3,18.9,19.6,24.7,
            "Ivan Duque",36.2,41.9,30.5,16.5,36.9,
            "Humberto de la Calle",3.4,5.9,1.2,5.4,3.9,
            "German Vargas Lleras",10.4,12.4,8.6,8.9,7.4)
...
fajardo_fit <- stan(file='fajardo.stan',data=list(N=9,N_encuestadora=6,int_voto=sf$int_voto,encuestadora=sf$encuestadora, muestra_int_voto=sf$muestra_int_voto,m_error=sf$m_error,dd=sf$dd,tipo=sf$tipo),control=list(adapt_delta=0.9),iter=4000,chains=4)
```
<br>
...
par <- posterior %>% as.tibble() %>% select(15:23) %>% summarize_all(funs(mean)) %>% t()
