...
## Plato
<br>
Estos son los resultados del modelo. Son promedios de la distribución posterior estimada para cada candidato, así como los intervalos HPD (higher posterior density) de 90% sobre esos parámetros. Para tener una referencia, el resultado se compara con dos promedios simples: uno sobre las 18 encuestas disponibles a la fecha que se han hecho durante 2018, y otro sobre las **11 encuestas** que se han hecho después de las elecciones legislativas del 11 de marzo.
...
plato <- tribble(~candidato,~int_voto,~int_voto_max,~int_voto_min,~m_2018,~m_03.2018,
            "Sergio Fajardo",13.4,16.1,10.7,13.7,11.6,
            "Gustavo Petro",22.7,24.5,21.0,22.7,26.2,
            "Ivan Duque",40.6,45.3,35.9,25.7,38.8,
            "Humberto de la Calle",2.3,3.8,0.8,4.5,3.3,
            "German Vargas Lleras",7.5,8.9,6.2,8.3,7.1)
...
fajardo_fit <- stan(file='fajardo.stan',data=list(N=11,N_encuestadora=6,int_voto=sf$int_voto,encuestadora=sf$encuestadora, muestra_int_voto=sf$muestra_int_voto,m_error=sf$m_error,dd=sf$dd,tipo=sf$tipo),control=list(adapt_delta=0.9),iter=4000,chains=4)
```
<br>
...
par <- posterior %>% as.tibble() %>% select(15:25) %>% summarize_all(funs(mean)) %>% t()
obs <- tibble(int_voto = sf$int_voto,
                n = seq(from=1,to=11,by=1))
